    ...
    for epoch in tqdm(range(30)):
        ...
        with torch.set_grad_enabled(True):
            for data in tqdm(trainloader):
                ...
                loss_crs_vel = criterion(outputs[:, 0, :], labels[:, 0, :])
                ...
                wandb.log({"tloss_crs_vel": loss_crs_vel})
        ...
        with torch.set_grad_enabled(False):
            for data in tqdm(validloader):
                ...
                for index in range(len(outputs[:, 0, :].tolist())):
                    ...
                    #                 })
                    valid_out.append({"o_crs_vel": outputs[:, 0, :].tolist()[index][0],
                                    "l_crs_vel": labels[:, 0, :].tolist()[index][0],
                                    })

                # vel, acc, crs, crs_velでMSE
                val_crs_vel += criterion(outputs[:, 0, :], labels[:, 0, :]) / len(validset)
        ...
        #                                                                                                             val_crs,
        #                                                                                                             val_crs_vel))
        print('#epoch:{}  train loss: {}  valid loss: {}  valid crs vel: {}'.format(epoch,
                                                                        running_train_loss,
                                                                        running_valid_loss,
                                                                        val_crs_vel,))
