        ...
        if ret:
            if cap.get(cv2.CAP_PROP_POS_FRAMES) == 1:  # 0秒のフレームを保存
                ...
                if second > 4:
                    ...
                    cv2.imwrite("{}_{}.{}".format(base_path, str(second), ext),
                                frame)
                    # 切り抜き
...
if __name__ == "__main__":
    ...
    with open(clip_file, 'r') as f:
        ...
        for row in tqdm(reader):
            ...
            _dir = os.path.join(frame_dir, '_' + row[0])
            save_frames("./ponnet_data/samples/" + file_name, _dir, file_name=row[0])